...
out(2)=(RaA) /Ixx; % roll accel [rad/s^2]

out(3)=dotpitch; % pitch rate [rad/s]
out(4)=(PaA) /Iyy; % pitch accel [rad/s^2]

out(5)=dotyaw; % yaw rate [rad/s]
out(6)=(YawA) /Izz;  % yaw accel [rad/s^2]

% TRANSLATIONS Z,X,Y
out(7)=dotz; % z rate [m/s]
out(8)=g + (-ZaA)/m;  % z accel [m/s^2]

out(9)=dotx;   % x rate [m/s]
out(10)=(-XaA)/m;  % x accel [m/s^2]

out(11)=doty;  % y rate [m/s]
out(12)=(-YaA)/m;  % y accel [m/s^2]
